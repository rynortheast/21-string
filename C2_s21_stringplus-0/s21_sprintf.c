...
int s21_sprintf(char * str, const char * format, ...) {
    ...
    for (int x = 0; format[x] != '\0'; x += 1) {
        if (format[x] == '%') {
            ...
            if (format[x] == 'n') {
                if (config.type == 'h')
                    *(va_arg(params, short int *)) = counter;
                else if (config.type == 'l')
                    *(va_arg(params, long int *)) = counter;
                else
                    *(va_arg(params, int *)) = counter;
            } else {
                counter += insertStringBySpecifier(str + counter, format[x], config, &params);
            }
        } else {
...
int searchModifiersForString(int x, const char * format, spec * config, va_list * params) {
    ...
    for (; s21_strchr("hlL", format[x]); x += 1)
...
int insertStringBySpecifier(char * str, char symbol, spec config, va_list * params) {
    ...
    if (symbol == '%') {
        ...
        if (config.type == 'h')
            *(va_arg(*params, short int *)) = s21_strlen(str);
        else if (config.type == 'l')
            *(va_arg(*params, long int *)) = s21_strlen(str);
        else
            *(va_arg(*params, int *)) = s21_strlen(str);
    } else if (symbol == 'c') {
        return s21_ctos(str + indent, va_arg(*params, int), flag, config.width);
        ...
        if (config.type == 'L')
            s21_ftoa_long(str + indent, va_arg(*params, long double), accuracy, flag, 0);
        else
            s21_ftoa(str + indent, va_arg(*params, double), accuracy, flag, 0);
    } else if (s21_strchr("gG", symbol)) {
        if (config.type == 'L')
            s21_gtoa_long(str + indent, va_arg(*params, long double), accuracy, flag, symbol);
        else
            s21_gtoa(str + indent, va_arg(*params, double), accuracy, flag, symbol);
    } else if (s21_strchr("eE", symbol)) {
        if (config.type == 'L')
            s21_ntoa_long(str + indent, va_arg(*params, long double), accuracy, flag, symbol, 0);
        else
        ...
        if (config.type == 'h')
            ...
            s21_utoa_long(str + indent, va_arg(*params, long int), symbol == 'x' ? 32 : 16, accuracy, flag);
        else
        ...
        if (config.type == 'h')
            ...
            s21_utoa_long(str + indent, va_arg(*params, long unsigned int), 8, accuracy, flag);
        else
        ...
        if (config.type == 'h')
            ...
            s21_utoa_long(str + indent, va_arg(*params, long unsigned int), 10, accuracy, flag);
        else
...
char * s21_conf(char * str, spec config, char symbol) {
    if (s21_strcmp(config.flag, "xxxxx") || config.width >= 0 || config.type != 'x') {
    ...
    if (countFill > 0) {
        for (s21_memmove(aux + countFill, aux, s21_strlen(aux) + 1); countFill != 0;) {
...
int s21_ctos(char * str, char symbol, char * flag, int accuracy) {
    ...
    for (int x = 0; flag[0] != 'o' && x < (accuracy - 1); x += 1)
        str[counter++] = ' ';
    str[counter++] = symbol;
    for (int x = 0; flag[0] == 'o' && x < (accuracy - 1); x += 1)
        str[counter++] = ' ';
...
char * s21_stos(char * str, char * data, int accuracy) {
    accuracy < 0 ? accuracy = s21_strlen(data) : 0;
    s21_strncat(str, data, accuracy);
    return str;
}
...
char * s21_ptoa(char * str, int * variable) {
    for (int *aux = variable, x = s21_strlen(str); aux != 0; aux = ((void*) (((size_t) aux) >> 4)), x += 1) {
...
char * s21_utoa(char * str, unsigned int number, int format, int accuracy, char * flag) {
    ...
    flag[2] == 'o' && format == 10 ? str[lenStr++] = ' ' : 0;
    (flag[3] == 'o' && format == 8) ? str[lenStr++] = '0' : 0;
    str[lenStr] = '\0';
    (flag[3] == 'o' && format == 16 && type == 65) ? s21_strcat(str, "X0") : 0;
    (flag[3] == 'o' && format == 16 && type == 97) ? s21_strcat(str, "x0") : 0;
    s21_reverse(str);
    return str;
}
...
char * s21_ntoa_long(char * str, long double number, int accuracy, char * flag, int symbol, int kostyl_2) {
    ...
    (kostyl != 0 || flag[3] == 'o') ? s21_strcat(str, ".") : 0;
    for (lenStr = s21_strlen(str); number < 0; number *= (-1))
        continue;
    for (result = lenNum; (accuracy != 0 && (lenNum + 1) <= 0); accuracy -= 1)
        if (accuracy == 1)
            s21_itoa(str + (lenStr++), fmodl((roundl(number) * powl(10, lenNum += 1)), 10), 1, "xxxxx");
        else
            s21_itoa(str + (lenStr++), fmodl((number * powl(10, lenNum += 1)), 10), 1, "xxxxx");
    for (int aux = lenNum + 1; accuracy != 0; accuracy -= 1)
        accuracy == 1 ? s21_itoa(str + (lenStr++), roundl(fmodl((number * powl(10, (aux++))), 10)), 1, "xxxxx") :
        s21_itoa(str + (lenStr++), fmodl((number * powl(10, (aux++))), 10), 1, "xxxxx");
...
char * s21_ftoa_long(char * str, long double number, int afterpoint, char * flag, int kostyl_2) {
    ...
    (kostyl != 0 || flag[3] == 'o') ? str[lenStr++] = '.' : 0;
    kostyl == 0 ? number = roundl(number) : 0;
    for (aux = roundl(number); ((aux / 10) >= 1); aux /= 10, str[lenStr] = '\0')
        str[lenStr++] = ((int) fmodl(aux, 10)) + 48;
    s21_itoa(str + lenStr, fmodl(aux, 10), 1, "xxxxx");
...
char * s21_ntoa(char * str, double number, int accuracy, char * flag, int symbol, int kostyl_2) {
    int lenStr = 0, lenNum = 0, result = 0, kostyl = accuracy;
    for (int aux = lenNum = fabs(number) < 1 ? 1 : fabs(number) < 10 ? 0 : (-1); aux != 0; lenNum += aux)
        aux = ((fabs(number) * pow(10, lenNum)) < 1 || 10 < fabs(number) * pow(10, lenNum)) ? aux : 0;
    s21_itoa(str, number * pow(10, lenNum), 1, flag);
    (kostyl != 0 || flag[3] == 'o') ? s21_strcat(str, ".") : 0;
    for (lenStr = s21_strlen(str); number < 0; number *= (-1))
        continue;
    for (result = lenNum; (accuracy != 0 && (lenNum + 1) <= 0); accuracy -= 1)
        accuracy == 1 ? s21_itoa(str + (lenStr++), fmod((round(number) * pow(10, lenNum += 1)), 10), 1, "xxxxx") :
        s21_itoa(str + (lenStr++), fmod((number * pow(10, lenNum += 1)), 10), 1, "xxxxx");
    for (int aux = lenNum + 1; accuracy != 0; accuracy -= 1)
        accuracy == 1 ? s21_itoa(str + (lenStr++), round(fmod((number * pow(10, (aux++))), 10)), 1, "xxxxx") :
        s21_itoa(str + (lenStr++), fmod((number * pow(10, (aux++))), 10), 1, "xxxxx");
    if (kostyl_2 == 1 && flag[3] != 'o') {
        for (int x = s21_strlen(str) - 1; s21_strchr("0.", str[x]); str[x--] = '\0')
            continue;
    }
...
char * s21_ftoa(char * str, double number, int afterpoint, char * flag, int kostyl_2) {
    ...
    for (aux = round(number); ((aux / 10) >= 1); aux /= 10, str[lenStr] = '\0')
        str[lenStr++] = ((int) fmod(aux, 10)) + 48;
    s21_itoa(str + lenStr, fmod(aux, 10), 1, "xxxxx");
    minus == 1 ? s21_strcat(str, "-") : 0;
    lenStr = s21_strlen(str);
    if (str[lenStr - 1] != '-' && (flag[1] == 'o' || flag[2] == 'o')) {
        str[lenStr++] = flag[1] == 'o' ? '+' : ' ';
